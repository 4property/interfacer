...
public class InterfacesAdder {
   ...
   * additional interfaces, it's possible that some classes may now match new
   * interfaces.
   *
   * <pre>
    ...
    if (interfacesDirectory == null && (interfacePackages == null || compileClasspathElements == null)) {
      throw new RuntimeException("""
          No interface source defined, received arguments:
              ...
              interfacePackages: %s
              languageLevel: %s
              compileClasspathElements: %s""".formatted(interfacesDirectory, interfacePackages, languageLevel,
  ...
  Modifications addInterfacesAllFiles(File scanDirectory, File interfacesDirectory, String interfacePackages,
      String languageLevel, List<String> compileClasspathElements,
      List<Pair<IfcResolve, ClassOrInterfaceDeclaration>> allModifications) throws IOException {

    Modifications modifications = parseAll(scanDirectory, interfacesDirectory, interfacePackages, languageLevel,
        compileClasspathElements);
  ...
  Modifications parseAll(File scanDirectory, File interfacesDirectory, String interfacePackages, String languageLevel,
    ...
    try {
      modifications = processAllFiles(source.tryToParse(), ifcs, parsingContext.javaParser);
    } catch (IOException e) {
      ...
      if (Log.isDebugEnabled()) {
        ...
        Log.warn(() -> "[InterfacesAdder] Failed to apply modifications to classes in "
            + scanDirectory.getAbsolutePath() + ". Cause: " + es.getMessage());
      }
   ...
   * <p>
   * If the argument <code>interfacesDirectory</code> is provided along with valid
   * <code>interfacePackages</code>, it will be scanned for all packages defined
   * in <code>interfacePackages</code> that match the directory structure.
   * <p>
   * If the argument <code>interfacesDirectory</code> is provided along with empty
   * or null <code>interfacePackages</code>, the directory will be passed back.
   * <p>
   * 
   * @param interfacesDirectory directory to scan for interfaces
   * @param interfacePackages   packages to scan for interfaces
   * @param languageLevel       language level to use when parsing source files
    ...
    if (interfacesDirectory == null) {
      if (interfacePackages == null || interfacePackages.isEmpty()) {
        throw new IllegalArgumentException("""
            No interface source defined, received arguments:
                interfacePackages: %s
                languageLevel: %s""".formatted(interfacePackages, languageLevel));
      }
      ...
      if (interfacePackages != null && !interfacePackages.isEmpty()) {
        ...
        for (String interfacePackage : interfacePackagesArray) {
          ...
          if (!packageDirectory.exists() || !packageDirectory.isDirectory()) {
            Log.warn(() -> "[InterfacesAdder] Package: %s not found in the given interfaces directory: %s, ignoring!"
                .formatted(interfacePackage, interfacesDirectory));
          }
        }
      }
  ...
  public List<IfcResolve> scanInterfaces(List<File> interfacesDirectories, String interfacePackages,
    ...
    uniqueResolvers.addAll(new ClasspathInterfacesScanner().scanInterfacesFromClasspath(parsingContext.classLoader,
        interfacePackages, parsingContext.typeSolver));
    uniqueResolvers.addAll(
        new SourceInterfacesScanner().scanInterfacesFromSrc(parsingContext.parserConfiguration, interfacesDirectories));
   ...
   */
  List<Pair<IfcResolve, ClassOrInterfaceDeclaration>> processAllFiles(List<ParseResult<CompilationUnit>> parseResults,
      List<IfcResolve> ifcs, JavaParser javaParser) {
    Log.info(
        () -> "[InterfacesAdder] Starting modification with " + parseResults.size() + " discovered java code file/s.");
    return flatMap(parseResults, parseResult -> {
      // Only deal with files without parse errors
      if (parseResult.isSuccessful()) {
        return parseResult.getResult().map(cu -> {
          if (Log.isDebugEnabled() && cu.getPrimaryTypeName().isPresent()) {
            Log.debug(() -> "[InterfacesAdder] Processing source java with compilation unit: "
                + cu.getPrimaryTypeName().get());
          } else if (Log.isDebugEnabled()) {
            Log.debug(() -> "[InterfacesAdder] Processing source java with available meta type: "
                + cu.getMetaModel().getTypeName());
          }
          // Do the actual logic
   ...
   * <code>ifc</code>. If true, add interface to the class. Return
   * <code>pair</code> representing interface and class. Return <code>null</code>
   * if interface was not added.
   */
  private Pair<IfcResolve, ClassOrInterfaceDeclaration> processDeclarationWithInterface(
    ...
    if (Log.isDebugEnabled()) {
      if (resolvedTypeVariables != null) {
        for (Entry<String, String> pair : resolvedTypeVariables.entrySet()) {
          Log.debug(() -> "[InterfacesAdder] Signature of type member: " + pair.getKey() + " matches to type: "
              + pair.getValue());
        }
      }
    }
    ...
    if (Log.isDebugEnabled() && canBeAssignedTo) {
      Log.debug(() -> "[InterfacesAdder] Inteface: " + ifc.getName() + " is already ancestor to type: "
          + declaration.getNameAsString() + ", skipping!");
    }
   ...
   * and add it to the class <code>declaration</code>.
   */
  private Pair<IfcResolve, ClassOrInterfaceDeclaration> addInterfaceToClassDeclaration(
    ...
    Log.info(() -> "[InterfacesAdder] Modifying the class: %s with interface: %s"
        .formatted(declaration.getFullyQualifiedName().orElse(""), ifc.getName()));

    ClassOrInterfaceType type = // new ClassOrInterfaceType(ifc.getName());
        javaParser.parseClassOrInterfaceType(ifc.getGenericName(resolvedTypeVariables)).getResult()
  ...
  private boolean canBeAssignedTo(ClassOrInterfaceDeclaration declaration, IfcResolve ifc) {
    return anyMatch(declaration.resolve().getAncestors(), ancestor -> {
      try {
